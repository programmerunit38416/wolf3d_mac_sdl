...
void RenderWallLoop(SignedWord x1,SignedWord x2,Word distance)
{
...
/* mark the segment as visible for auto map*/
	...
	rw_scale = (SignedLongWord)ScaleFromGlobalAngle(start+centershort,distance)<<FRACBITS;
	if (x2>x1+1) {
		scale2 = (SignedLongWord) ScaleFromGlobalAngle(stop+centershort,distance)<<FRACBITS;
		rw_scalestep = (SignedLongWord)(scale2-rw_scale)/(SignedLongWord)(x2-x1);
	}
...
void DrawOverheadSeg (saveseg_t *seg)
{
	{
		// East/West
		if (seg->dir&1)
		{	
			...
			int end = seg->max ;
			int plane = seg->plane ;
			DrawLine (start, plane, end, plane, 20);
		}
		else // North/South
		{	
			int start = seg->min ;
			int end = seg->max ;
			int plane = seg->plane ;
			DrawLine (plane, start, plane, end, 20);
		}
	}
}
...
{
	VideoPointer[y * SCREENWIDTH + x] = color;
}
...
/**********************************
	...
	PWW:
	NOTE: To allow for negative pixel valies for walls, use clipshortangle2 instead of clipshortangle
