#include "wolfdef.h"
#include <string.h>
#include <stdio.h>   /* For FILE, fopen, fwrite, fclose */
#include <stdlib.h>  /* For system() and getenv() */
#include "Wolf.h"    /* For rGamePal definition */
...
void ExportToBmp(SignedLongWord index, Byte* data);
static Boolean LoadWallShape(Word Index,Byte *DarkPtr)
{
	...
	/*
    	...
    	This is how its loaded:
			WallListPtr = (unsigned short *) LoadAResource(MyWallList);

		The WallListPtr is BIG Endian!! And after masking with 3fff the
		resources correspond with the wall files starting from 300

		Bit 15 is the 'dark' bit.

		The first byte is '64' and seems to indicate the number wall indexes
		in this list.

		The problem is in BRGR there are only files for 36 walls, so the
		remaining walls must be somewhere else in the code.
		SUCESS!!!
		Because of the 'dark' bit wall files are duplicated if the dark
		bit is set. 27 have the dark bit set, 8 don't which gives us 
		64 walls. 
		All wall data looks accounted for.


		Walls are 128x128 pixels!
		0x4000 = 128 x 128
	*/

	WallVal = BigToLittleEndianShort(WallListPtr[Index+1]);		/* Which resource is it? */
...
/************************************
	...
	Export a sprite to a BMP file
	Sprites are stored in a special column-based format
	This function will create a bitmap representation

************************************/
Word GetSpriteHeight(Word SpriteWidth, unsigned short *CharPtr)
{
	Word height = 0;
	SpriteRun *RunPtr;
	for (Word column = 0; column < SpriteWidth; column++) {
		RunPtr = (SpriteRun *)&CharPtr[CharPtr[column+1] / 2]; // Get the offset to the RunPtr data
		while (RunPtr->Topy != 0xFFFF) {
			height = RunPtr->Boty > height ? RunPtr->Boty : height;
			RunPtr++;
		}
	}
...
void DrawSpriteColumnToBuffer(Word column, Word SpriteWidth, unsigned short *CharPtr, Byte *OutBuffer)
{
	SpriteRun *RunPtr;
	Byte *CharPtr2;
    ...
    RunPtr = (SpriteRun *)&CharPtr[CharPtr[column + 1] / 2]; // Get the offset to the RunPtr data
	Byte* Buffer = &OutBuffer[column];

    while (RunPtr->Topy != 0xFFFF) 
	{ 
        int Y1 = RunPtr->Topy;
		int Y2 = RunPtr->Boty;
		int Index = RunPtr->Shape + (RunPtr->Topy / 2);
		int Delta = 0;
		int RunCount = Y2 - Y1; // How many lines to draw?
		if (RunCount)
		{
			// Draw the scaled sprite segment
			Byte *SrcPtr = &CharPtr2[Index];
			Byte *DestPtr = &Buffer[Y1*SpriteWidth];
...
void ExportSpriteToBmp(Word index, Byte* data)
{
    char filename[256];
    FILE *fp;
    Word width, height;
    SignedLongWord filesize;
    Word runs;
    
    
    char mkdir_cmd[256];
    sprintf(mkdir_cmd, "mkdir -p %s/Documents/temp/sprites", getenv("HOME"));
    system(mkdir_cmd);
    sprintf(filename, "%s/Documents/temp/sprites/sprite_%03d.bmp", getenv("HOME"), index);
    
    // Determine sprite dimensions
    runs = data[0];
    width = runs;  // Width is the number of columns
    
    // Fixed height for sprite images
    height = GetSpriteHeight(width, (Word*)data);  // Standard height for Wolf3D sprites

	unsigned short *patch;
	patch = SpriteArray[index];		/* Get the info on the shape */
	width = patch[0];		/* Adjust the width */

	Word column = 0;					/* Start at the first column */

	Byte *imgData = (Byte*)malloc(width * height);
	if (!imgData) {
		printf("Failed to allocate memory for sprite image\n");
		return;
	}
	memset(imgData, 0, width * height);

	do {
		DrawSpriteColumnToBuffer(column, width, patch, imgData);
	} while (++column < width);
    ...
    // Calculate filesize (header + pixel data)
    ...
    Byte* palette = LoadAResource(rGamePal); /* Set the current palette */
    ...
    // Write pixel data (convert indexed color to RGB)
    for (SignedLongWord y = height - 1; y >= 0; y--) {  // BMP is stored bottom-to-top
        for (SignedLongWord x = 0; x < width; x++) {
            Byte color_index = imgData[y * width + x];
            
			// If palette is in RGB order but you're writing as BGR
			unsigned char r = palette[color_index * 3];     // Red (first byte)
			unsigned char g = palette[color_index * 3 + 1]; // Green (second byte)
			unsigned char b = palette[color_index * 3 + 2]; // Blue (third byte)

			// Then write in BGR order for BMP
			fwrite(&b, 1, 1, fp);
			fwrite(&g, 1, 1, fp);
			fwrite(&r, 1, 1, fp);
        }
    ...
    // Cleanup
    fclose(fp);
    printf("Exported sprite %d to %s\n", index, filename);
}
/************************************

	ExportToBmp 
	For testing
	Saves to ~/Documents/temp/
...
void ExportToBmp(SignedLongWord index, Byte* data)
{
    char filename[256];
    FILE *fp;
    SignedLongWord width = 128;
    SignedLongWord height = 128;
    SignedLongWord filesize = 54 + 3 * width * height;  // BMP header + RGB data
    
    // Make sure we have the game palette loaded
    ...
    // Create filename with path to ~/Documents/temp
    sprintf(filename, "%s/Documents/temp/wall_%03d.bmp", getenv("HOME"), index);
    
    // Create directory if it doesn't exist
    char mkdir_cmd[256];
    sprintf(mkdir_cmd, "mkdir -p %s/Documents/temp", getenv("HOME"));
    system(mkdir_cmd);
    ...
    for (SignedLongWord y = height - 1; y >= 0; y--) {  // BMP is stored bottom-to-top
        for (SignedLongWord x = 0; x < width; x++) {
            SignedLongWord pixel_index = y * width + x;
            unsigned char color_index = data[pixel_index];
            
            // Get RGB values from palette (assuming palette format is R,G,B)
            unsigned char b = CurrentPal[color_index * 3];     // Blue
            unsigned char g = CurrentPal[color_index * 3 + 1]; // Green
            unsigned char r = CurrentPal[color_index * 3 + 2]; // Red
            
            // Write BGR (BMP format stores as BGR)
            fwrite(&b, 1, 1, fp);
            fwrite(&g, 1, 1, fp);
            fwrite(&r, 1, 1, fp);
        }
        // Padding for 4-byte alignment (each row must be multiple of 4 bytes)
        fwrite(bmppad, 1, (4 - (width * 3) % 4) % 4, fp);
    }
    
    fclose(fp);
    printf("Exported wall %d to %s\n", index, filename);
}
...
Word LoadSpriteArt(void)
{
	...
	do {
		if (WallHits[i]) {
			...
			Length |= MyPtr[1]<<8;
#else
			Length = MyPtr[1];				/* Get the length unpacked */
			Length |= MyPtr[0]<<8;
#endif
			...
			SpriteArray[i] = (Word*)MyNewPtr;		/* Save the pointer */

			// PWW
			// Convert sprite data from BIG to CORRECT endianness
			// Change the first two bytes to little endian
			SpriteArray[i][0] = BigToLittleEndianShort(SpriteArray[i][0]);
			Word runs = SpriteArray[i][0];
			for (int j = 0; j < runs; j++) {
				SpriteArray[i][1+j] = BigToLittleEndianShort(SpriteArray[i][1+j]);
			}

			// PWW
			// Convert column run medata to correct endianness
			for (Word col = 0; col < runs; col++) {
				Word offset = SpriteArray[i][col+1] / 2;  // Convert from byte offset to word offset
				SpriteRun *RunPtr = (SpriteRun *)&SpriteArray[i][offset];

				while (BigToLittleEndianShort(RunPtr->Topy) != (unsigned short)-1) {
					RunPtr->Topy = BigToLittleEndianShort(RunPtr->Topy);
					RunPtr->Boty = BigToLittleEndianShort(RunPtr->Boty);
					RunPtr->Shape = BigToLittleEndianShort(RunPtr->Shape);
					RunPtr++;
				}
			}

			//ExportSpriteToBmp(i, MyNewPtr);	
