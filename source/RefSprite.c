...
void RenderSprite(Word x1,Word x2,vissprite_t *VisPtr)
{
	...
	if ((SignedLongWord) x1 > VisPtr->x1) {		/* Clip the left? */
...
void AddSprite (thing_t *thing,Word actornum)
{
	...
	SignedLongWord px;				/* Center X coord */
	unsigned short *patch;	/* Pointer to sprite data */
	SignedLongWord x1, x2;			/* Left,Right x */
...
/* calculate edges of the shape */
	...
	if (x1 >= (SignedLongWord) SCREENWIDTH) {
...
void DrawSprites(void)
{
	...
	SignedLongWord x1,x2;				/* Left x, Right x */
	Word i;					/* Index */
...
/* draw from smallest scale to largest */
		...
		do {
			...
			x2 = dseg->x2;
			if (x2>= (SignedLongWord)SCREENWIDTH) {	/* Clip the right? */
				x2 = SCREENWIDTH-1;
			}
