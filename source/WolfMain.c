...
/* find angle to face attacker */
	...
	// NOTE:
	// Previous code used: DestAngle = PointToAngle(killx,killy)&(-1<<SHORTTOANGLESHIFT);	
	// 1111 1111 1111 1111  (-1 in binary)
	// Shift left by 7 gives:
	// 1111 1111 1000 0000  (lower 7 bits are 0) 
	// Changed to use complement of right shift to mask off the SHORTTOANGLESHIFT bits.
	DestAngle = PointToAngle(killx,killy) & ~((1<<SHORTTOANGLESHIFT)-1); /* What's the direction of the kill angle */	
...
#ifdef DEV_LEVELSTART
	gamestate.mapon = DEV_LEVELSTART;
#else
	gamestate.mapon = 1;
#endif
...
#ifdef DEV_GODMODE 
	gamestate.godmode = 1;		/* Force god mode on */
#else
