...
typedef struct {
	Byte Area1;	/* First area */
	Byte Area2;	/* Second area */
} connect_t;
...
typedef struct {
	...
	Word pwalldir;
	SignedWord pwallxchange, pwallychange;  	/* adjust coordinates this much*/
} pushwall_t;
...
typedef struct {
	unsigned short *pos;    /* position of sprite info - changed from void* for 64-bit compatibility */
	ufixed_t columnstep;	/* Step factor for width scale */
	SignedWord x1,x2;				/* Left x, Right x */
...
typedef struct {		/* Must match thing_t */
	...
	SignedWord xspeed,yspeed;	/* Direction of travel */
} missile_t;
...
extern SignedLongWord	rw_scale;
extern SignedLongWord	rw_scalestep;
extern Word rw_midpoint;
extern Word rw_mintex;
extern Word rw_maxtex;
extern Byte *rw_texture;
extern SignedWord rw_centerangle;
...
extern SignedWord UnclippedViewAngleToX(Word angle);
...
extern void RenderWallLoop(SignedWord x1,SignedWord x2,Word distance);
...
extern unsigned short *SpriteArray[S_LASTONE];			/* Pointers to all the sprites */
...
extern Byte *GameShapes[MAXGAMESHAPES];		/* Pointer to the game shape array */
...
extern SignedWord clipshortangle;				/* Angle for the left edge of the screen */
extern SignedWord clipshortangle2;				/* clipshortangle * 2 */
...
/* Global palette data */
extern Byte CurrentPal[768];  /* 256 colors * 3 components (R,G,B) */


extern Word QuitFlag;       // Set to 1 to quit
