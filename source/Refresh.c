...
#ifdef DRAWBSP
#include <stdio.h>
SignedLongWord drawsegcount, checksegcount, rwallcount;
#endif
...
Word ScaleFromGlobalAngle(SignedLongWord visangle,SignedLongWord distance)
{
	fixed_t	tz;
...
/* viewangle tangent table*/
	...
	if (MathSize==0xffff) {		/* Only needs to be done once */
		i = 0;
		do {
...
/* use tangent table to generate viewangletox*/
	...
	do {
		...
		t = (SignedLongWord)SCREENWIDTH/2 - t;
		if (t < -1) {
			t = -1;
		} else if (t>(SignedLongWord)SCREENWIDTH+1) {
			t = SCREENWIDTH+1;
		}
		viewangletox[i] = t;
	} while (++i<FINEANGLES/2);
...
#ifdef DRAWBSP
	fixed_t px = actors[0].x >> (FRACBITS-1);
	fixed_t py = actors[0].y >> (FRACBITS-1);
