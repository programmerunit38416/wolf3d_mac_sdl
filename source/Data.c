...
unsigned char *GameShapes[MAXGAMESHAPES];			/* Pointer to the game shape array */
...
SignedWord clipshortangle;					/* Angle for the left edge of the screen */
SignedWord clipshortangle2;				/* clipshortangle * 2 */
...
Word rw_maxtex;
Word rw_mintex;
SignedLongWord rw_scalestep;
Word rw_midpoint;
Boolean rw_downside;
SignedWord rw_centerangle;
Byte *rw_texture;
SignedLongWord rw_scale;
Byte *ArtData[64];
unsigned short *SpriteArray[S_LASTONE];  /* Changed from void* for 64-bit compatibility */
Word MacVidSize = (Word)-1;
...
/* Global palette data */
Byte CurrentPal[768];  /* 256 colors * 3 components (R,G,B) */
