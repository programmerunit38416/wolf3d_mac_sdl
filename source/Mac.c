...
void InitTools(void)
{
    ...
    if (Gestalt(gestaltVersion, &Feature))
    {                /* Is gestalt working? */
        goto Not607; /* Oh oh... */
    }
    ...
    if (Gestalt(gestaltSystemVersion, &Feature) ||
        (short)Feature < 0x0607)
    { /* Is it 6.0.7 or later? */
        goto Not607;
    }
    ...
    if (!gMainGDH)
    {              /* No macs that can display or canceled */
        GoodBye(); /* Exit */
    }
    OldPixDepth = (**(**gMainGDH).gdPMap).pixelSize; /* Get the previous mode */
    if (OldPixDepth != 8)
    { /* Was it 8 bit already? */
        if (SetDepth(gMainGDH, 8, 1 << gdDevType, 1))
        {              /* Set to 8 bit */
            BailOut(); /* Oh oh... */
        }
    }
...
static void ResetPalette(void)
{
    ...
    if ((**(**gMainGDH).gdPMap).pixelSize != 8)
...
Word NewGameWindow(Word NewVidSize)
{
    ...
    { /* Is this 640 mode? */
        ...
        {
            if (MonitorHeight < 480)
            {                   /* Can I display 480 lines? */
                NewVidSize = 2; /* Show 400 instead */
            }
        }
    }
    if (NewVidSize == MacVidSize)
    {                      /* Same size being displayed? */
...
TryAgain:
    ...
    if (Pass2)
    { /* Low memory? */
        if (!StartupRendering(NewVidSize))
        { /* Reset the scalers... */
            ReleaseScalers();
            goto OhShit;
        }
...
void ReadSystemJoystick(void)
{
    ...
    {                   /* Was a key hit? */
        ...
        switch (ScanCode)
        {          /* Use the SCAN code to make sure I hit the right key! */
        case 0x12: /* 1 */
    ...
    if (joystick1 & JOYPAD_TR)
    { /* Handle the side scroll (Special case) */
...
Word ChooseGameDiff(void)
{
    ...
    do
    {
        ...
        case 4: /* The checkbox */
            ...
            if (DblClick == itemHit)
            { /* Hit the same item? */
                if (((Word)ReadTick() - OldTick) < (Word)15)
                { /* Double click time? */
                    donewithdialog = TRUE;
                    difficulty = OldVal - 1;
                    break; /* Ok! */
...
void SaveGame(void)
{
    ...
    if (HOpen(SaveFileVol, SaveFileParID, SaveFileName, fsWrPerm, &FileRef))
    {           /* Can I open it? */
        return; /* Abort! */
    }
...
Boolean LoadGame(void)
{
    ...
    FSClose(FileRef);                                   /* Close the file */
    if (memcmp(MachType, FilePtr.B, 4))
    { /* Is this the proper machine type? */
        goto Bogus;
    }
    FilePtr.B += 4; /* Index past signature */
    if (MapListPtr->MaxMap != *FilePtr.S)
    {               /* Map count the same? */
...
static Word DoMyAlert(Word AlertNum)
{
    ...
    **AlertRect = MyRect;
    ...
    if (!InPause)
    { /* If paused then don't fix the cursor */
        FixTheCursor();
    }
    return Val;
}
...
static void CenterSFWindow(Point *MyPoint, Word Width, Word Height)
{
    ...
    if (gMainGDH)
    {                                                                 /* Has the video device been selected? */
...
void InitAEStuff(void)
{
    ...
    if (Gestalt(gestaltAppleEventsAttr, &aLong))
    { /* Are Apple events present? */
        return;
    }
