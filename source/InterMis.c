...
/* setup */
	...
	ShapePtr = LoadAndDecompressResource(rIntermission);
	if (!ShapePtr) {
		return;
	}
	...
	BJPtr = LoadAndDecompressResource(rInterPics);
	if (!BJPtr) {
		return;
	}
	memcpy(Indexs,BJPtr,12);		/* Copy the index table */
	LongWord* BJLongPtr = (LongWord*)BJPtr;
	Indexs[0] = BigToLittleEndian(BJLongPtr[0]);
	Indexs[1] = BigToLittleEndian(BJLongPtr[1]);
	Indexs[2] = BigToLittleEndian(BJLongPtr[2]);
...
/* If 100% on all counts, Perfect Bonus! */
	...
	do {
		ShowBJ();		/* Animate BJ */
	} while (!WaitTicksEvent(10));		/* Wait for a keypress */
